			...
			<div class="sorting-left">
				<form
					...
					<select id="itemNumber" name="numberitem"
						onchange="this.form.submit()" class="frm-field required sect">
	...
	<nav class="numbering animated wow slideInRight" data-wow-delay=".5s">
		<ul class="pagination paging">
			<c:forEach var="i" begin="1" end="${numberOfPages}">
				<li id="pageNumber_${i}">
					<a href="javascript:void(0);" onclick="viewproductpageajax(${i});">
						<c:out value="${i}" />
					</a>
				</li>
			</c:forEach>
...
<script type="text/javascript">
			...
			function viewproductpageajax(pageNumber) {
				var xmlHttpRequest = getXMLHttpRequest();
				xmlHttpRequest.onreadystatechange = getReadyStateHandler(xmlHttpRequest);
				...
				xmlHttpRequest.open("GET",
						"${pageContext.request.contextPath}/views/viewproductpageajax"
								+ queryString, true);
				xmlHttpRequest.send();
			}
				...
				return function() {
					...
					if (xmlHttpRequest.readyState == 4) {
						if (xmlHttpRequest.status == 200) {
							...
							if (jsonFunction == "jsonfunction_products") {
			...
			$(document).ready(function() {
				...
				viewproductpageajax(1);
				
				
				// danh dau * cho latest product
				$('span.stars').stars();
			...
			//danh dau * rating cho list products ajax
